...
def receive_external_data():
    ...
    # -------------------------
    # NEW: If this is a motion/PIR device and payload contains 'motion',
    # then ignore temperature/humidity and set data to Motion Detected / No Motion.
    # Always set No Threat and emit update so dashboard shows correct data immediately.
    # This prevents temp/humidity strings from showing in the PIR card.
    # -------------------------
    try:
        dev_type_lower_local = (str(device.get('type')).lower() if device.get('type') else "")
        is_motion_device_local = 'motion' in dev_type_lower_local or 'pir' in dev_type_lower_local
        if is_motion_device_local and ('motion' in payload):
            # parse motion value robustly
            mv = payload.get('motion')
            try:
                mv_int = int(mv)
            except Exception:
                mv_int = 1 if str(mv).strip().lower() in ('true', '1', 'yes', 'on') else 0

            data_str = "Motion Detected" if mv_int else "No Motion"
            threat_status = "No Threat"

            # update DB and emit (always emit so UI updates even if threat unchanged)
            cur = conn.execute('SELECT threat FROM devices WHERE id = ?', (device_id,)).fetchone()
            cur_threat = cur['threat'] if cur else None
            ...
            try:
                ...
                print("Socket emit error (motion-only branch):", e)

            conn.close()
            return jsonify({"status": "processed", "threat": threat_status})
    except Exception as e:
        print("Motion-branch error:", e)
        # fall through to normal processing if something unexpected happens
    ...
    try:
        ...
        if same_count >= REPLAY_REPEAT_THRESHOLD:
            threat_status = "Threat Detected"
            # if device is motion device but we reached here (no motion field) use payload-based data
            if ('motion' in payload) and (('motion' in (device['type'] or "").lower() or 'pir' in (device['type'] or "").lower())):
                try:
                    mv = int(payload.get('motion', 0))
                    data_str = "Motion Detected" if mv else "No Motion"
                except Exception:
    ...
    # For final DB update, if this is a motion device and payload included motion, use Motion/No Motion;
    # otherwise show temperature/humidity string (unchanged behavior for weather devices).
    if ('motion' in payload) and (('motion' in (device['type'] or "").lower() or 'pir' in (device['type'] or "").lower())):
        try:
            mv = int(payload.get('motion', 0))
            data_str = "Motion Detected" if mv else "No Motion"
